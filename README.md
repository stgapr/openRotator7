- # 卫星天线旋转器开源固件 openRotator7
  ...
  本项目 openRotator7 是 **DTrac Rotor V1** 卫星天线旋转器的开源替代固件，基于 **SARCNET Mini Rotator MK1** 的原始开源代码“Rotator7”进行移植和增强。保留了原始固件的核心控制算法，同时增加了网络配置和用户交互功能。

  > **重要声明**：本固件适用于 DTrac Rotor V1 硬件设计，但**不属于 DTrac 官方项目的一部分**。它由第三方开发，旨在为 DTrac 硬件用户提供一个开源、可控、可定制的固件选项。所有功能均基于公开的硬件资料和开源协议实现。
  ...
  - **电机驱动**：L298N 或兼容的 H 桥驱动板（默认为 ESP12E motor shield 板，板载 L298N 驱动）
  ...
  > **使用场景**：当您使用外置姿态传感器（如 WT9011DCL-BT50）时，上位机（如 DTrac App）通过蓝牙或串口获取传感器数据并解算出当前角度，然后通过 `XZ/XL` 命令上报给旋转器。旋转器收到后，会结合之前通过 `AZ/EL` 设定的目标角度，自行计算误差并驱动电机。
...
> 如使用外部传感器， 调试模式和校准模式不可用。
  ...
  | **`XZ<角度>`**      | `XZ135.5`        | **上报外置方位角**（仅外置传感器模式）                       | `\n`   |
  | **`XL<角度>`**      | `XL42.0`         | **上报外置仰角**（仅外置传感器模式）                         | `\n`   |
